    ...
    # condition when article(s) are found 
        ...
        for article in idList[0:5]: 
            ...
            try: 
                ...
                names = str(author_list).replace("'", "").replace("[", "").replace("]", "").replace(",", ".,")
                ...
                print(f"{names} ({pub_date[0:4]}) '{corrected_title}', {journal}, {volume}({issue}), pp.{pages}. Available at:{doi} (Accessed: {d8})")
                print("------------------------")
                input("Completed ")
            # create exception 
            except: 
...
print("Harvard Citation Generator - For USW - by openwolf.com")
print("----------------")
term = input("Enter DOI or title: ").replace(" ", "+")
